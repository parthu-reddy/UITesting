...
| MENU-03 | Multiple brands listed | On the customer home page, verify that at least two distinct brand names are shown in the restaurant list. | Implemented; at least two cards with distinct nonempty brand names must render. Cover-image-specific validation remains pending. |
...
| MENU-08 | Item image fallback | If an item has no image, a placeholder or brand logo should be shown. | Pending product behavior: current menu source omits the image region when `imageUrl` is absent; rendered images use decorative empty alt text. |
| MENU-09 | Item description | If an item has a description, it is rendered below the name in smaller text. | Partial implementation live-passed: every row is checked for leaked `null` or `undefined`; description-specific styling remains pending. |
