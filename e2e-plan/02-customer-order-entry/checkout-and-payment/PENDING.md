...
`CheckoutUiTest.paymentChoicesAndCheckoutReentry` now covers CHECKOUT-07 through CHECKOUT-10 using only visible controls: Card and UPI must be enabled and selectable, Wallet is selected when the displayed account makes it available, closing preserves the exact cart item, and reopening restores all three method tiles with an enabled Pay action. Live validation is pending while `/api/delivery/verification/status` returns HTTP 503 and prevents the required rider-availability setup.
